...
def set_need_appearances_writer(writer: PdfWriter) -> None:
    ...
    try:
        ...
        if CatalogDictionary.ACRO_FORM not in catalog:
            writer._root_object[NameObject(CatalogDictionary.ACRO_FORM)] = writer._add_object(DictionaryObject())

        need_appearances = NameObject(InteractiveFormDictEntries.NeedAppearances)
        writer._root_object[CatalogDictionary.ACRO_FORM][need_appearances] = BooleanObject(True)  # type: ignore
